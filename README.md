...
## 느낀점
쇼핑몰을 만드는 것이 뭐 별거 있겠어? 라는 생각을 한것은 오산이었다. 실제로 만들어보니 고려해야 할 점이 한두가지가 아니었던 것 같다. 혼자 구글링도 해보고 유튜브 영상도 찾아보면서 이론을 공부했을 때는 이해하는데 큰 어려움이 없었는데 막상 이러한 프로젝트를 해보니 너무 쉽게 생각했던 것 같다. 또한 버그 발생으로 코드를 수정해야할 때가 있었는데 내가 짠 코드를 다시 살펴봤을 때 한눈에 이해하기 힘들었던 적이 있었다. 깔끔한 코드로 수정해야겠다. 웹사이트를 완성하고 끝내는 것이 아니라 지인이나 가족에게 사용을 요구해서 추가적으로 필요한 기능이 있는지, 혹은 불편한 점이 있는지 조사해서 웹사이트에 반영해봐야겠다.
